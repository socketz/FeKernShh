...
namespace FeKernShh
{
    class FilterParser
    {
        ...
        public static void WalkRegistryKeys(out string altitude, out string altName, ref string keyName)
        {
            altitude = null;
            altName = null;
            ...
            foreach (string service in services.GetSubKeyNames())
            {
                ...
                foreach (string subkeyName in serviceSubkey.GetSubKeyNames())
                {
                     if (subkeyName.Contains("Instances"))
                    {
                        ...
                        {
                            if (subsubkeyName.Equals(keyName))
                            {
                                RegistryKey serviceAltitude = servicesSubsubkey.OpenSubKey(subsubkeyName);
                                altitude = serviceAltitude.GetValue("Altitude").ToString();
                                altName = service;
                                
                                /*
                                foreach (string valueName in serviceAltitude.GetValueNames())
                                {
                                    if (valueName.Equals("Altitude"))
                                    {
                                        altitude = serviceAltitude.GetValue("Altitude").ToString();
                                        altName = service;
                                    }                                    
